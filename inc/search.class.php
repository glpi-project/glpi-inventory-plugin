...
class PluginGlpiinventorySearch extends CommonDBTM
{
    ...
    * @return boolean
    */
    public function constructSQL($itemtype, $params, $items_id_check = 0)
    {
        global $CFG_GLPI;
       ...
       // Add Restrict to current entities
        if ($entity_restrict) {
            ...
            } elseif (isset($CFG_GLPI["union_search_type"][$itemtype])) {
           ...
           // Link items tables
            for ($i = 0; $i < $_SESSION["glpisearchcount2"][$itemtype]; $i++) {
                if (
                    ...
                    if (
                        !in_array(
                            $searchopt[$p['itemtype2'][$i]][$p['field2'][$i]]["table"] . "_" .
                                 $p['itemtype2'][$i],
                            $already_link_tables2
                        )
                    ) {
                ...
                if (
                    ...
                    // Manage Link if not first item
                        if (
                            ...
                            $tmplink = " " . str_replace(" NOT", "", $p['link2'][$key]);
                            $NOT     = 1;
                        } elseif (is_array($p['link2']) && isset($p['link2'][$key])) {
                            $tmplink = " " . $p['link2'][$key];
                        } else {
                            $tmplink = " AND ";
                        }
                    ...
                    if ($citem->canView()) {
                        ...
                        if ($itemtype == 'States') {
                            $query_num = str_replace(
                                $CFG_GLPI["union_search_type"][$itemtype],
                                $ctable,
                                $tmpquery
                            );
                          ...
                          // Remove template items
                            ...
                            $query_num = str_replace(
                                "FROM `" . $CFG_GLPI["union_search_type"][$itemtype] . "`",
                                $replace,
                                $tmpquery
                            );
                            $query_num = str_replace(
