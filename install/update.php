...
function pluginGlpiinventoryGetCurrentVersion()
{
    ...
    if (
        (!$DB->tableExists('glpi_plugin_tracker_config')) && (!$DB->tableExists('glpi_plugin_fusioninventory_config')) && (!$DB->tableExists('glpi_plugin_fusioninventory_configs')) && (!$DB->tableExists('glpi_plugin_glpiinventory_configs'))
    ) {
        return '0';
    } elseif (
        ($DB->tableExists('glpi_plugin_tracker_config')) || ($DB->tableExists('glpi_plugin_glpiinventory_config'))
    ) {
        ...
        if (
            (!$DB->tableExists('glpi_plugin_tracker_agents')) && (!$DB->tableExists('glpi_plugin_fusioninventory_agents'))
        ) {
            return '1.1.0';
        }
        if (
            (!$DB->tableExists('glpi_plugin_tracker_config_discovery')) && (!$DB->tableExists('glpi_plugin_fusioninventory_config'))
        ) {
            return '2.0.0';
        }
        if (
            (($DB->tableExists('glpi_plugin_tracker_agents')) && (!$DB->fieldExists('glpi_plugin_tracker_config', 'version'))) && (!$DB->tableExists('glpi_plugin_fusioninventory_config'))
        ) {
            return '2.0.1';
        }
        if (
...
function pluginGlpiinventoryUpdate($current_version, $migrationname = 'Migration')
{
   ...
   */
    ...
    $a_input['server_upload_path'] = Toolbox::addslashes_deep(
        implode(
            DIRECTORY_SEPARATOR,
            [
                GLPI_PLUGIN_DOC_DIR,
...
function do_biosascomponentmigration()
{
    ...
    //BIOS as a component
    if (
        $DB->tableExists('glpi_plugin_glpiinventory_inventorycomputercomputers') && ($DB->fieldExists('glpi_plugin_glpiinventory_inventorycomputercomputers', 'bios_date') || $DB->fieldExists('glpi_plugin_glpiinventory_inventorycomputercomputers', 'bios_version') || $DB->fieldExists('glpi_plugin_glpiinventory_inventorycomputercomputers', 'bios_manufacturers_id'))
    ) {
        $bioses = [];
        //retrieve exiting
