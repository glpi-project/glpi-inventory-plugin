...
function pluginGlpiinventoryUpdateNative($current_version, $migrationname = 'Migration')
{
   ...
   //update to new ids
    ...
    if (!$DB->tableExists('glpi_plugin_glpiinventory_ipranges_snmpcredentials')) {
   ...
   //update to new ids
    ...
    $migration->displayMessage("Use core unmanaged equipments"); //agents must be migrated before that one
    $DB->queryOrDie(
    ...
    $migration->displayMessage("Use core refused equipments"); //agents must be migrated before that one
    $DB->queryOrDie(
    ...
    if ($DB->tableExists('glpi_plugin_glpiinventory_rulematchedlogs')) {
        $migration->displayMessage("Use core rules logs"); //agents must be migrated before that one
        $DB->queryOrDie(
    ...
    $migration->displayMessage("Use core remote management"); //agents must be migrated before that one
    $DB->queryOrDie(
