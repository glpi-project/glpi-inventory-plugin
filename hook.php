            ...
            if (is_array($condition)) {
                $condition = array_filter(
                    array_map(
                        static function ($sub) {
                            if (is_array($sub)) {
                                // delete only QueryExpression in sub array with value "0" -> which generate WHERE(0) in SQL clause
                                return array_values(array_filter(
                                    $sub,
                                    static fn($val) => !(
                                        $val instanceof QueryExpression
                                        && method_exists($val, 'getValue')
                                        && $val->getValue() === "0"
                                    )
                                ));
                            }
                    ...
                    static fn($sub) => $sub !== [] // delete empty sub array
                );
            } else {
                $condition = [];
            }
